...
class SuperPointFrontend(object):
    ...
    def nms_fast(self, in_corners, H, W, dist_thresh):
        ...
        count = 0
        # one cell (pad *2, pad * 2) should only has one corner
        for i, rc in enumerate(rcorners.T):
    ...
    def run(self, img):
        ...
        # Remove dustbin. (65, 15, 20) => (64, 15, 20)
        nodust = dense[:-1, :, :]
...
def draw_keypoints(img, corners, color, radius=8, s=3):
    img = np.repeat(cv2.resize(img, None, fx=s, fy=s)[..., np.newaxis], 3, -1)
    for c in np.stack(corners).T:
        cv2.circle(img, tuple(s * np.flip(c, 0)), radius, color, thickness=-1)
    return img

def select_top_k(corners, num=300):
    corners = corners.astype(int)
    idx = np.argsort(-corners)[::-1][0][:num]
    pts = (corners[0][idx], corners[1][idx])
    return pts
