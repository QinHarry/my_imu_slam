...
void IMU::euler(const MotionData& cur, const Eigen::Quaterniond& Qwb, Eigen::Quaterniond& dq, Eigen::Vector3d& acc_w) {
    Eigen::Vector3d gw(0,0,-9.81);
    double dt = param_.imu_timestep;
    Eigen::Vector3d dtheta_half =  cur.imu_gyro * dt /2.0;
    dq.w() = 1;
    dq.x() = dtheta_half.x();
    dq.y() = dtheta_half.y();
    dq.z() = dtheta_half.z();
    ...
    acc_w = Qwb * (cur.imu_acc) + gw;  // aw = Rwb * ( acc_body - acc_bias ) + gw
}


void IMU::midPoint(const MotionData& prev, const MotionData& cur, const Eigen::Quaterniond& Qwb, Eigen::Quaterniond& dq, Eigen::Vector3d& acc_w) {
    Eigen::Vector3d gw(0,0,-9.81);
    double dt = param_.imu_timestep;

    Eigen::Vector3d imu_gyro = (prev.imu_gyro + cur.imu_gyro) / 2.0;
    Eigen::Vector3d dtheta_half =  imu_gyro * dt /2.0;
    dq.w() = 1;
    dq.x() = dtheta_half.x();
    dq.y() = dtheta_half.y();
    dq.z() = dtheta_half.z();
    ...
    acc_w =  (Qwb * prev.imu_acc + Qwb * dq * cur.imu_acc) / 2.0 + gw;
}
...
void IMU::testImu(std::string src, std::string dist)
{
    ...
    for (int i = 1; i < imudata.size(); ++i) {
        ...
        std::cout << "start process " << i << " imudata " << std::endl;
        MotionData imupose = imudata[i];

        Eigen::Quaterniond dq;
        Eigen::Vector3d acc_w;
        //euler(imupose, Qwb, dq, acc_w);
        midPoint(imudata[i-1], imudata[i], Qwb, dq, acc_w);
